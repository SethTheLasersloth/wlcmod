...
// Clientbound
public class LockInputsPackets {
...
//        PayloadTypeRegistry.playS2C().register(WLC_Mod.BytePayload2.ID2, WLC_Mod.BytePayload2.CODEC);
    ...
    public static void registerHandlers(){
        //? <=1.20.1 {
        ClientPlayNetworking.registerGlobalReceiver(new Identifier (WLC_Mod.MOD_ID, WLC_Mod.path_LockMovement), (client, handler, buf, responseSender) -> {
            int duration = buf.readInt();  // Read duration from the packet
            client.execute(() -> WLC_Mod.setLockTicks(duration));
        });
        ClientPlayNetworking.registerGlobalReceiver(new Identifier(WLC_Mod.MOD_ID, WLC_Mod.path_UnlockMovement), (client, handler, buf, responseSender) -> {
